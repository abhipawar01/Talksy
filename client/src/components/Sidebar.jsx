...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
    ...
    const deleteChat = async (e, chatId) => {
      try {
        e.stopPropagation()
          ...
          const {data} = await axios.post(`/api/chat/deletechat/${chatId}`, {chatId}, {
  ...
  return (
    ...
    ${!isMenuOpen && 'max-md:translate-x-full'}`}>
        ...
        {/*New Chat Button*/}
        <button onClick={createNewChat} className='flex justify-center items-center w-full py-2 mt-10 text-white bg-gradient-to-r from-[#A456F7] to-[#7C3AED]
        ...
        <div className='felx-1 overflow-y-scroll mt-3 text-sm space-y-3'>
          {
            ...
            includes(search.toLowerCase())).map((chat)=> (
              ...
              key={chat._id} className='p-2 px-4 dark:bg-[#573176C]/10 border
        ...
        {/*Community Images*/}
        <div onClick={()=> {navigate('/community'); setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-auto border border-gray-300
        ...
        dark:border-white/15 rounded-md '>
         ...
         {/*User Profile*/}
         <div className='flex items-center gap-3 p-3 mt-auto border border-gray-300
        dark:border-white/15 rounded-md cursor-pointer group'>
          ...
          <p className='flex-1 text-sm dark:text-primary truncate'>{user ? user.name :
            ...
            {user && <img onClick={logout} src={assets.logout_icon} className='h-4 cursor-pointer hidden not-dark:invert 
            group-hover:block' alt=""/>} 
        </div>
